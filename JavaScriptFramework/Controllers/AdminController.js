    ...
    static async HandleAdmin(req, res){
        ...
        users.map(u => {
            ...
            if (ban) {
                ...
                usersHMTL += `${banUntilDate.getUTCFullYear()} - ${banUntilDate.getUTCMonth() + 1} - ${banUntilDate.getUTCDate()}`
            }
            usersHMTL += `
                </td>
                <td>`
        ...
        res.render('admin', {
            ...
            DATA: req.session.note !== "<p><br></p>" && req.session.note ? req.session.note : ''
        })
