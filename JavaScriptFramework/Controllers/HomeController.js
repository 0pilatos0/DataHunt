...
module.exports = class HomeController extends Controller{
    ...
    static async HandleHome(req, res){
        ...
        res.render('index', {
            ...
            PATCHDATE: `${parsedDate.getDate()}/${parsedDate.getMonth()+1}/${parsedDate.getFullYear()} - ${parsedDate.getHours()}:${parsedDate.getMinutes()}`
        })
    }
    ...
    static async HandlePostRegister(req, res){
        ...
        let password = Salter.hashPassword(InputParser.parse(req.data["AccPassword"]))
        ...
        if(!name.match(Regex.Name)){
            returnErrorMessage(`<div class=\"alert alert-danger\" role=\"alert\">You are the type of person to have a red line under your name in word.</div>`)
            return
        }
        if(!username.match(Regex.Username)){
            returnErrorMessage(`<div class=\"alert alert-danger\" role=\"alert\">Your username doesn't follow our rules!</div>`)
            return
        }
        if(!email.match(Regex.Email)){
            returnErrorMessage(`<div class=\"alert alert-danger\" role=\"alert\">This isn't a valid email adress!</div>`)
            return
        }
        if(!req.data["AccPassword"].match(Regex.password)){
            returnErrorMessage(`<div class=\"alert alert-danger\" role=\"alert\">Your password should be 8 characters long, have one uppercase and lowercase letters and a number!</div>`)
            return
        }
        if(req.data["AccPassword"] !== req.data["AccPasswordCheck"]){
            returnErrorMessage(`<div class=\"alert alert-danger\" role=\"alert\">Passwords don't match!</div>`)
            return
        }
        ...
        if(id){
            returnErrorMessage(`<div class=\"alert alert-danger\" role=\"alert\">Username already in use</div>`)
            return
        }
        await User.create({
            name,
        ...
        mailPage.vars.HOST = `${process.env.HOST}${process.env.PORT ? `:${process.env.PORT}` : ''}`
